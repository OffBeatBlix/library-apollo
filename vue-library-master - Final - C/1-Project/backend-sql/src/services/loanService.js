...
module.exports = class LoanService {
  ...
  async create(data) {
    ...
    try {
      ...
      try {
      await this.bookRepository.refreshStock(record.bookId, {
        transaction,
        currentUser: this.currentUser,
      ...
      if (error instanceof ValidationError) {
        throw new ValidationError(this.language, 'entities.loan.validation.bookOutOfStock');
      }
  ...
  async sendEmails(ids) {
    const  { rows: loans } = await this.repository.findAndCountAll({
      filter: {
        ids,
      },
    ...
    const overdueLoansEmails = loans
      ...
      .map(
        (loan) => new LoanInProgressEmail(this.language, loan),
      );
