...
class BookRepository extends AbstractRepository {
  ...
  async findAndCountAll(
    ...
    if (filter) {
      ...
      if (filter.callNumber) {
        sequelizeFilter.appendIlike('callNumber', filter.callNumber, 'book');
      }
      ...
      if (filter.edition) {
        sequelizeFilter.appendIlike('edition', filter.edition, 'book');
      }

      if (filter.publisher) {
        sequelizeFilter.appendIlike('publisher', filter.publisher, 'book');
      }

      if (filter.publisher) {
        sequelizeFilter.appendIlike('copyright', filter.copyright, 'book');
      }

      if (filter.accession_number) {
        sequelizeFilter.appendIlike('accession_number', filter.accession_number, 'book');
      }

      if (filter.circulationType) {
        sequelizeFilter.appendEqual('circulationType', filter.circulationType);
      }
  ...
  async findAllAutocomplete(query, limit) {
    ...
    if (query) {
      ...
      filter.appendIlike('callNumber', query, 'book');
      filter.appendIlike('author', query, 'book');
      filter.appendIlike('title', query, 'book');
    }
    //Tags
    const records = await models.book.findAll({
     // attributes: ['id','callNumber', 'author', 'title'],
     attributes: ['id','callNumber', 'author', 'title', 'stock'],
    ...
    return records.map((record) => ({
      ...
      label: `${record.callNumber} - ${record.title
      }`,  
      stock: record.stock,
    }));
  }
  ...
  async refreshStock(id, options) {
    ...
    const stock = Number(book.numberOfCopies) - Number(booksNotInStock);

    if (stock < 0) {
      throw new ValidationError(options.language, 'entities.book.validation.bookOutOfStock',);
    }
